	...
	f=fopen(file,"rb");
     ...
     if(f==NULL) {
	  ...
	  if((filesize<0x10)||(filesize>0x4000000))
	  {
		  fclose(f);
#ifdef LINUX
__android_log_print(ANDROID_LOG_INFO, "ManyChip", "HAError: %s", "Bad File Size");
...
//	  fread(&header,1,4,f);
	  ...
	  if(header!=0x22465350)
	  {
		  fclose(f);
#ifdef LINUX
__android_log_print(ANDROID_LOG_INFO, "ManyChip", "HAError: %s", "Bad Header");
			  printf("Bad header\n");
#endif
		  return gsffile;
	  }
...
//	  fread(&ccrc,1,4,f);
	  ...
	  if((reserved+program+16)>filesize)
	  {
		  fclose(f);
#ifdef LINUX
__android_log_print(ANDROID_LOG_INFO, "ManyChip", "HAError: %s", "Incoherant sizes");
	  ...
	  if(reserved>0)
	  {
		  ...
		  if(gsffile.reserved==NULL)
		  {
			  ...
			  __android_log_print(ANDROID_LOG_INFO, "ManyChip", "HAError: %s %d", "Malloc failed", reserved);
#endif
			  return gsffile;
		  }
...
bool utildecompGSF(const char * file)
{
	...
	__android_log_print(ANDROID_LOG_INFO, "ManyChip", "HAError: %s", "Failed to load");
	...
	if(!psftag_raw_getvar(gsffile.psftag,"_lib",libtag,sizeof(libtag)-1)) {
		...
		if(gsflib[1].gsfloaded == false)
		{
			...
			__android_log_print(ANDROID_LOG_INFO, "ManyChip", "HAError: %s", "Failed to load library");
			free(uncompbuf);
			return false;
		}
