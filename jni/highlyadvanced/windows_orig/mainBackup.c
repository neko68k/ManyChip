...
#define ProgName "Highly Advanced v0.01"
#define AppVer   ProgName " by Zoopd and CaitSith2.\nBased on the Visual Boy Advance code.\nOriginal PSF concept by Neill Corlett."
...
DWORD IgnoreTrackLength=0, DefaultLength=1;
DWORD deflen=180,deffade=4,silencelength=5,silencedetected=0;
...
{
	int ret = soundBufferLen;		//*2 cause it's in shorts, result will be 576
	
	//while(BigBufferOffset+soundBufferLen >= sizeof(BigBuffer))		//while the BigBuffer is too full to take in the new buffer
	//	bBigBufferFull = 1;											//the big buffer is full, so we'll add the data after we output  

	while (mod.outMod->CanWrite() < ((soundBufferLen*sndNumChannels*(sndBitsPerSample/8))<<(mod.dsp_isactive()?1:0)))
		Sleep(50);
	...
	//decode_pos_ms+=(soundBufferLen*1000)/sndSamplesPerSec;
	...
	if (mod.dsp_isactive())
		...
		ret = mod.dsp_dosamples((short *)soundFinalWave, ret / sndNumChannels / (2), sndBitsPerSample, sndNumChannels, sndSamplesPerSec) * (sndNumChannels * (2));
